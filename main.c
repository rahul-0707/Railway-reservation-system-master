...
void bysource(void);
void sorttrains(void);
void specialtraindisplay(void);
...
void modifyadmin()
{
 struct node *r,*s;
        ...
        if(loc==1)
    {
        ...
        writeadmin();
        free(r);
        return; // changed from return(headd);
    }
    else{s=headd;}
    for(i=1;i<loc-1;i++)
    {
...
{   struct node *p,*q;
	    ...
	    if(loc==1)
    {
        ...
        displaysortedtrains();
        free(p);
        return; // changed from return(head);
    }
    else{q=head;}
    for(i=1;i<loc-1;i++)
    {
